...
def gameLoop():		
	...
	while not gameExit: #Stay in this loop unless the game is quit
		if mainBoard.isNewPiece():
			...
			aiPlayer.changeVariableProgram(matrix,currentPiece)
			obj = aiPlayer.execute()
			minCol = mainBoard.rotate(int(obj.get_rotation()))
			if currentPiece == "o":
				minCol = 4
			col = int(obj.get_col())
			n = col - minCol
			if n>0:
				mainBoard.move(True,False,n)
			elif n<0:
				mainBoard.move(False,True,abs(n))
