...
class MainBoard:
    ...
    def draw_SCOREBOARD_CONTENT(self):
        ...
        if self.gameStatus == 'running':
            ...
            for i in range(0, 4):
                ...
                if self.nextPieces[1] == 'O':
                    self.draw_BLOCK(xPosRef + 0.5 * self.blockSize, yPosRef + 2.25 * self.blockSize + 15,
                                    blocks[i][ROW], blocks[i][COL], blockColors[self.nextPieces[1]])
                elif self.nextPieces[1] == 'I':
                    self.draw_BLOCK(xPosRef + 0.5 * self.blockSize, yPosRef + 1.65 * self.blockSize + 15,
                                    blocks[i][ROW], blocks[i][COL], blockColors[self.nextPieces[1]])
                else:
                    self.draw_BLOCK(xPosRef + 1 * self.blockSize, yPosRef + 2.25 * self.blockSize + 15, blocks[i][ROW],
        ...
        ntetrisText = fontSB.render('Adduci-Doria', False, TEXT_COLOR)
...
# Main program
Recognizer.checkVersion = lambda *args, **kwags: None
